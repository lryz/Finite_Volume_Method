...
def euler_backward(equation):
    ...
    Returns
    -------
        decomposed equation after replacing du(x,t)/dt by ((ux,tn+1)-u(x,tn))/deltat and all others t by tn+1
    """
    left = equation[0]
    for i in range(0,len(left)):
        for j in range(0,len(left[i])):
            if type(left[i][j]) == sp.core.function.Derivative :
                if left[i][j].args[1][0] == t :
                    #replacing du(x,t)/dt by ((ux,tn+1)-u(x,tn))/deltat
                    var_implicit =type(left[i][j].args[0])
                    left[i][j] = (left[i][j].args[0].subs({t : tnplusun}) - left[i][j].args[0].subs({t : tn}))/Deltat
    right=equation[1]
    for i in range(0,len(right)):
        for j in range(0,len(right[i])):
            #replacing t by tn+1
            if type(right[i][j])==var_implicit and (right[i][j].args[0] == xi or right[i][j].args[0] == xiplusun or right[i][j].args[0] == ximoinsun):
                right[i][j]= right[i][j].subs({t : tnplusun})
            else :
                right[i][j]= right[i][j].subs({t : tn})
    return [left,right]